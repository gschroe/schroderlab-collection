 ...
 * and writes out the resulting density map in MRC format.
 *
 * The program supports an optional template MRC file (to obtain grid parameters)
 * and additional command-line options: -angpix, -box, -center, -res (resolution cutoff)
 * and now -k to set the conversion factor for B-factor broadening.
 *
 * Rather than scaling the density contribution with an ad hoc factor exp(–B/50),
 * the B-factor (given in Å²) is used to broaden the Gaussian kernel. For each atom
 * an effective kernel width is computed as:
 *
 *     σ_eff = sqrt( σ² + B / k )
 *
 * Here k is a tunable constant (default 8.0). Physically, in Fourier space the B‐factor
 * causes a damping exp(–B·s²/4), and B is related to the mean-square displacement via
 *     B = 8π²⟨u²⟩
 *
...
/* Global Peng parameters (adapted from render.c :contentReference[oaicite:2]{index=2}&#8203;:contentReference[oaicite:3]{index=3}) */
...
/* pDenPengKernel: Computes the density contribution given the squared distance r,
   an atom type index, and an effective kernel width sigma. Here we modify the
   exponential argument by dividing by sigma so that a larger sigma (i.e. more blurring)
   results in a softer drop-off.
*/
double pDenPengKernel(double r, int type, double sigma) {
    double h0, h1, h2, h3, h4, v;
    double sf[5], sf_a[5];
    h0 = -9.8696 * r / sigma;   // Incorporate sigma into the scaling of r.
    ...
    sf[4] = PengScatterFactor_B[type][3];  // Use same as index 3 for pass5.
...
/* pDenRenderPeng: Renders the density map using the Peng kernel.
   For each atom, an effective kernel width is computed as:
       σ_eff = sqrt( σ² + B / k )
   where σ is the intrinsic width (given as input), B is the atom’s B-factor (in Å²)
   stored in the factors array, and k is a tunable constant.
   The effective kernel is then applied over the local grid.
*/
PDen_t * pDenRenderPeng(PDen_t * result, double sigma, double width, size_t n, int *type, double *coords, double *factor, double k_value) {
    size_t atom;
    size_t idx, idxx;
    size_t i, j, k;
    ...
    /* Do not square sigma here; instead, use its value directly */
    ...
    for (atom = 0; atom < n; atom++) {
        ...
        mass = 1.0;  // mass is incorporated via the kernel broadening, so we no longer multiply by factor after
        type_ = type[atom];
        
        /* Compute effective sigma (intrinsic plus B-factor broadening) */
        double B = factor[atom];  // Here factor[atom] holds the B-factor (in Å²)
        double sigma_eff = sqrt(sigma * sigma + B / k_value);
                    ...
                    if (j < result->size.y) {
                        for (i = gridid.x; i < gridend.x; i++) {
                            if (i < result->size.x) {
                                ...
                                /* Use effective sigma in the kernel call */
                                density = pDenPengKernel(lenr, type_, sigma_eff);
...
/* get_atom_index: Maps an atom type string to an integer index.
   Mapping (example):
...
/* count_atoms_in_pdb: Returns the number of ATOM/HETATM lines in the PDB file. */
...
/* parse_pdb: Parses the PDB file and allocates arrays for coordinates, atom type indices, and B-factors.
   Reads B-factor from columns 61-66 and stores the value (in Å²) in factors.
   Also trims the atom type field and maps it to an index.
*/
size_t parse_pdb(const char *pdb_file, double **p_coords, int **p_type, double **p_factor) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        if (strncmp(line, "ATOM", 4) == 0 || strncmp(line, "HETATM", 6) == 0) {
            char x_str[9], y_str[9], z_str[9], atype[5] = "", trimmed_atype[5] = "";
            char b_str[7] = "";
            ...
            z_str[8] = '\0';
            // Extract atom type from columns 77-78 or fallback to columns 13-16.
            if (strlen(line) >= 78) {
            ...
            // Read B-factor from columns 61-66.
            if (strlen(line) >= 66) {
                strncpy(b_str, line + 60, 6);
                b_str[6] = '\0';
            }
            double B = atof(b_str);
            factors[atom] = B;  // Store B in Å²
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        fprintf(stderr, "Usage: %s input.pdb output.mrc [template.mrc] [-angpix value] [-box nx ny nz] [-center cx cy cz] [-res resolution] [-res_edge width] [-k value] [-mask_out file.mrc] [-mask_radius value] [-mask_cushion value] [-mask_edge value] [-mask_smooth sigma]\n", argv[0]);
        return 1;
    }
    ...
    double k_value = 8.0;     // default conversion factor for B-factor broadening
    ...
    printf("B-factor conversion k value: %f\n", k_value);
    
    /* Parse the PDB file into coordinate, type, and B-factor arrays */
    double *coords = NULL;
    int *atom_types = NULL;
    double *factors = NULL;
    ...
    /* Render density map using pDenRenderPeng.
       Default intrinsic kernel width: sigma = 2.0 Å.
       Default walk width: width = 10.0 Å.
    */
    double sigma = 2.0;
    double width = 10.0;
    pDenRenderPeng(&density_map, sigma, width, natoms, atom_types, coords, factors, k_value);
